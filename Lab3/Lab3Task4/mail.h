...
void PrintMail(Mail *mail) {
    ...
    if (mail->GetStatus) {
        printf("The mail is recieved\n");
    }
    else {
        printf("The mail is waiting\n");
    }
...
Address *CreateAddress(char *city, char *street, unsigned short house, unsigned short flat, char *building,
                       char *postIndex) {
    Address *result = (Address*)malloc(sizeof(Address));
    if (!result) {
        return NULL;
    }
...
Mail *CreateMail(Address *address, double weight, char *id, char *create, char *receive, bool getstatus) {
    Mail *result = (Mail*)malloc(sizeof(Mail));
    if (!result) {
        return NULL;
    }
